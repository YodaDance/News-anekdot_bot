...
def anekdot_script(anekdots, n_of_page):
    # ретернит лист анекдотов с конкретной страницы
    url = 'https://humornet.ru/anekdot/evrei/page/{}/'
    if not anekdots:  # если лист пустой
        # собирает информацию со страницы, начиная с 1ой страницы
        response = requests.get(url.format(n_of_page))
        response.encoding = 'utf-8'
        # парсит страницу в html формат
...
def balaboba_script(text):
    # ретернит строку после ввода вашего текста в сервис балабобу
    url = "https://zeapi.yandex.net/lab/api/yalm/text3"
    ua = UserAgent().ff
    headers = {'Content-Type': 'application/json', 'User-Agent': ua,
               'Origin': 'https://yandex.ru', 'Referer': 'https://yandex.ru/'}
